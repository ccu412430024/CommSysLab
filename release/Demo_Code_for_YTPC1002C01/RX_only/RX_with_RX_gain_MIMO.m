...
Number_of_RX_rows=2;  % 1:SISO, 2:MIMO
rx_len=100000;
% set_RX_Gain([0 20; 1 20]);
Pow_com = set_RX_Ref_Level_ELSDR([0 -10; 1 -8])
rk = RX(Number_of_RX_rows, rx_len);
P_rk1=10*log10(mean(abs(rk(1,:)).^2))-Pow_com(1)
P_rk2=10*log10(mean(abs(rk(2,:)).^2))-Pow_com(2)


Fs=122.88e6;
figure(1)
P1=plot(0:size(rk(1,:),2)-1,real(rk(1,:)),'b',0:size(rk(1,:),2)-1,imag(rk(1,:)),'b.-',0:size(rk(2,:),2)-1,real(rk(2,:)),'r',0:size(rk(2,:),2)-1,imag(rk(2,:)),'r.-');
...
rk_f1=fftshift(fft(rk(1,:),FFT_SIZE));
rk_f2=fftshift(fft(rk(2,:),FFT_SIZE));
P2=plot(xf,20*log10(abs(rk_f1)),'b',xf,20*log10(abs(rk_f2)),'r');
