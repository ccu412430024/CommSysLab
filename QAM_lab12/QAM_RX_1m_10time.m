%% 16-QAM Receiver for SDR (10 Iterations)
clc; clear; close all;
...
% 建立繪圖視窗
figure('Name', '16-QAM SDR Result - 10 Times', 'Color', 'w', 'Position', [100, 100, 1500, 600]);
num_iterations = 10;

for iter = 1:num_iterations
    fprintf('\n========================================\n');
    fprintf('--- 開始執行第 %d/%d 次 SDR RX ---\n', iter, num_iterations);
    ...
    try
        ...
        % 輸出結果
        fprintf('第 %d 次接收成功! Msg: %s  SNR: %.2f dB (EVM: %.2f%%)\n', iter, rx_str, est_snr, evm_rms_pct);
        ...
        title(sprintf('#%d (SNR: %.2f dB)', iter, est_snr)); 
        xlabel('I'); ylabel('Q'); 
        grid on; axis square;
        xlim([-2 2]); ylim([-2 2]);
        
    catch ME
        % 捕捉錯誤，避免單次解碼失敗導致整個迴圈中斷
        fprintf('第 %d 次接收或解碼失敗：%s\n', iter, ME.message);
        
        subplot(2, 5, iter);
        text(0.5, 0.5, 'Failed', 'HorizontalAlignment', 'center', 'Color', 'r', 'FontSize', 12);
        title(sprintf('#%d Error', iter));
        axis off;
    end
    
    % 暫停一小段時間，確保硬體有緩衝時間處理下一次接收指令
    pause(0.5); 
end
fprintf('\n--- 10次測試執行完畢 ---\n');